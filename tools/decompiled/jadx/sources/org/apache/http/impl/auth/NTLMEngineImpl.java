...
final class NTLMEngineImpl implements NTLMEngine {
    ...
    public class CipherGen {
        ...
        public byte[] getTimestamp() {
            if (this.timestamp == null) {
                long jCurrentTimeMillis = 10000 * (System.currentTimeMillis() + 11644473600000L);
    ...
    public static byte[] lmHash(String str) throws NTLMEngineException {
        try {
            byte[] bytes = str.toUpperCase(Locale.US).getBytes(CharEncoding.US_ASCII);
            byte[] bArr = new byte[14];
            System.arraycopy(bytes, 0, bArr, 0, Math.min(bytes.length, 14));
            Key keyCreateDESKey = createDESKey(bArr, 0);
            Key keyCreateDESKey2 = createDESKey(bArr, 7);
            byte[] bytes2 = "KGS!@#$%".getBytes(CharEncoding.US_ASCII);
            Cipher cipher = Cipher.getInstance("DES/ECB/NoPadding");
            cipher.init(1, keyCreateDESKey);
    ...
    class NTLMMessage {
        ...
        NTLMMessage(String str, int i) throws NTLMEngineException {
            this.messageContents = null;
            this.currentOutputPosition = 0;
            this.messageContents = Base64.decode(EncodingUtils.getBytes(str, NTLMEngineImpl.DEFAULT_CHARSET), 0);
        ...
        String getResponse() {
            ...
            return EncodingUtils.getAsciiString(Base64.encode(bArr, 0));
        }
    }
    ...
    class Type1Message extends NTLMMessage {
        ...
        Type1Message(String str, String str2) throws NTLMEngineException {
            try {
                ...
                this.hostBytes = strConvertHost != null ? strConvertHost.getBytes(NTLMEngineImpl.DEFAULT_CHARSET) : null;
                this.domainBytes = strConvertDomain != null ? strConvertDomain.toUpperCase(Locale.US).getBytes(NTLMEngineImpl.DEFAULT_CHARSET) : null;
            } catch (UnsupportedEncodingException e2) {
        ...
        String getResponse() {
            ...
            addUShort(261);
            addULong(2600);
            addUShort(Sequence.EXPORT_UHD_WIDTH);
    ...
    class MD4 {
        ...
        protected void processBuffer() {
            int[] iArr = new int[16];
            for (int i = 0; i < 16; i++) {
                iArr[i] = (this.dataBuffer[i * 4] & UnsignedBytes.MAX_VALUE) + ((this.dataBuffer[(i * 4) + 1] & UnsignedBytes.MAX_VALUE) << 8) + ((this.dataBuffer[(i * 4) + 2] & UnsignedBytes.MAX_VALUE) << 16) + ((this.dataBuffer[(i * 4) + 3] & UnsignedBytes.MAX_VALUE) << 24);
            }
            int i2 = this.A;
            int i3 = this.B;
            int i4 = this.C;
