...
public class XmlStreamReader extends Reader {
    ...
    private String doLenientDetection(String str, XmlStreamReaderException e2) throws IOException {
        if (str != null && str.startsWith(MimeTypes.TEXT_HTML)) {
            try {
                return calculateHttpEncoding(MimeTypes.TEXT_XML + str.substring(MimeTypes.TEXT_HTML.length()), e2.getBomEncoding(), e2.getXmlGuessEncoding(), e2.getXmlEncoding(), true);
            } catch (XmlStreamReaderException e3) {
                e2 = e3;
            }
        }
        String xmlEncoding = e2.getXmlEncoding();
        if (xmlEncoding == null) {
            xmlEncoding = e2.getContentTypeEncoding();
        }
        if (xmlEncoding == null) {
    ...
    private static String getXmlProlog(InputStream inputStream, String str) throws IOException {
        ...
        return matcher.group(1).toUpperCase().substring(1, r0.length() - 1);
    }

    static boolean isAppXml(String str) {
        return str != null && (str.equals(AdobeNetworkHttpRequestParameters.HTTP_CONTENT_TYPE_APPLICATION_XML) || str.equals("application/xml-dtd") || str.equals("application/xml-external-parsed-entity") || (str.startsWith("application/") && str.endsWith("+xml")));
    }
