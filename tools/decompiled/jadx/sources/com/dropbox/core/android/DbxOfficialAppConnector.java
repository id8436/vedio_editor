...
public class DbxOfficialAppConnector {
    ...
    public class DbxOfficialAppInstallInfo {
        ...
        public String toString() {
            return String.format("DbxOfficialAppInstallInfo(versionCode=%s, supportsOpenWith=%s)", Integer.valueOf(this.versionCode), Boolean.valueOf(this.supportsOpenWith));
        }
    }
    ...
    private static int getLoggedinState(Context context, String str) {
        Cursor cursorQuery = context.getContentResolver().query(LOGGED_IN_URI.buildUpon().appendPath(str).build(), null, null, null, null);
        if (cursorQuery == null) {
            return 0;
        }
        cursorQuery.moveToFirst();
        return cursorQuery.getInt(cursorQuery.getColumnIndex("logged_in"));
    }
    ...
    static PackageInfo getDropboxAppPackage(Context context, Intent intent) {
        ...
        if (listQueryIntentActivities == null || 1 != listQueryIntentActivities.size() || (resolveInfoResolveActivity = packageManager.resolveActivity(intent, 0)) == null) {
            return null;
        }
        try {
            PackageInfo packageInfo = packageManager.getPackageInfo(resolveInfoResolveActivity.activityInfo.packageName, 64);
    ...
    public static Intent generateOpenWithIntentFromUtmContent(String str) throws DropboxParseException {
        try {
            ...
            throw new DropboxParseException("UtmContent was not base64 encoded: " + e2.getMessage());
        }
    }
}
