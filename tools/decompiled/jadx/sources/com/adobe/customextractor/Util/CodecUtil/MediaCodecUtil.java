...
public class MediaCodecUtil {
    ...
    public static DecoderInfo getDecoderInfo(String str, boolean z, boolean z2) throws DecoderQueryException {
        Pair<String, MediaCodecInfo.CodecCapabilities> mediaCodecInfo = getMediaCodecInfo(str, z, z2);
    ...
    public static synchronized void warmCodec(String str, boolean z, boolean z2) {
        try {
            getMediaCodecInfo(str, z, z2);
        } catch (DecoderQueryException e2) {
            Log.e(TAG, "Codec warming failed", e2);
        }
    }

    private static synchronized Pair<String, MediaCodecInfo.CodecCapabilities> getMediaCodecInfo(String str, boolean z, boolean z2) throws DecoderQueryException {
        ...
        if (codecs.containsKey(codecKey)) {
            ...
            mediaCodecInfo = getMediaCodecInfo(codecKey, Utilities.SDK_INT >= 21 ? new MediaCodecListCompatV21(z) : new MediaCodecListCompatV16(), z2);
            if (z && mediaCodecInfo == null && Utilities.SDK_INT >= 21) {
                Pair<String, MediaCodecInfo.CodecCapabilities> mediaCodecInfo2 = getMediaCodecInfo(codecKey, new MediaCodecListCompatV16(), z2);
    ...
    private static Pair<String, MediaCodecInfo.CodecCapabilities> getMediaCodecInfo(CodecKey codecKey, MediaCodecListCompat mediaCodecListCompat, boolean z) throws DecoderQueryException {
        try {
            return getMediaCodecInfoInternal(codecKey, mediaCodecListCompat, z);
        } catch (Exception e2) {
            throw new DecoderQueryException(e2);
        }
    }

    private static Pair<String, MediaCodecInfo.CodecCapabilities> getMediaCodecInfoInternal(CodecKey codecKey, MediaCodecListCompat mediaCodecListCompat, boolean z) {
        ...
        for (int i = 0; i < codecCount; i++) {
            ...
            if (!codecInfoAt.isEncoder() && name.startsWith("OMX.") && ((zSecureDecodersExplicit || !name.endsWith(".secure")) && (!z || name.contains(".sw.")))) {
                for (String str2 : codecInfoAt.getSupportedTypes()) {
                    if (str2.equalsIgnoreCase(str)) {
                        ...
                        if (!zSecureDecodersExplicit) {
                            codecs.put(codecKey.secure ? new CodecKey(str, false, z) : codecKey, Pair.create(name, capabilitiesForType));
                            if (zIsSecurePlaybackSupported) {
                                codecs.put(codecKey.secure ? codecKey : new CodecKey(str, true, z), Pair.create(name + ".secure", capabilitiesForType));
                            }
                        } else {
                            codecs.put(codecKey.secure == zIsSecurePlaybackSupported ? codecKey : new CodecKey(str, zIsSecurePlaybackSupported, z), Pair.create(name, capabilitiesForType));
                        }
                        if (codecs.containsKey(codecKey)) {
    ...
    private static boolean isAdaptive(MediaCodecInfo.CodecCapabilities codecCapabilities) {
        return Utilities.SDK_INT >= 19 && isAdaptiveV19(codecCapabilities);
    }
    ...
    public static boolean isH264ProfileSupported(int i, int i2) throws DecoderQueryException {
        Pair<String, MediaCodecInfo.CodecCapabilities> mediaCodecInfo = getMediaCodecInfo("video/avc", false, false);
        if (mediaCodecInfo == null) {
            return false;
        }
    ...
    final class MediaCodecListCompatV21 implements MediaCodecListCompat {
        ...
        @Override // com.adobe.customextractor.Util.CodecUtil.MediaCodecUtil.MediaCodecListCompat
    ...
    final class CodecKey {
        ...
        public int hashCode() {
            return (((this.secure ? 1231 : 1237) + (((this.mimeType == null ? 0 : this.mimeType.hashCode()) + 31) * 31)) * 31) + (this.software ? 1231 : 1237);
        }
