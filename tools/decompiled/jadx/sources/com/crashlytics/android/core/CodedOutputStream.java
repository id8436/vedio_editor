...
final class CodedOutputStream implements Flushable {
    ...
    public void writeRawBytes(ByteString byteString, int i, int i2) throws IOException {
        ...
        InputStream inputStreamNewInput = byteString.newInput();
        if (i4 != inputStreamNewInput.skip(i4)) {
            throw new IllegalStateException("Skip failed.");
        }
        while (i5 > 0) {
            int iMin = Math.min(i5, this.limit);
            int i6 = inputStreamNewInput.read(this.buffer, 0, iMin);
            if (i6 != iMin) {
